...
Мониторинг панели Remnawave, её нод, доступности нод из РФ и прокси к Telegram Bot API с алертами в Telegram.
...
- **Telegram-прокси** (каждый цикл, если задан `TG_PROXY_URL`): `GET {TG_PROXY_URL}/bot<токен>/getMe`.
  Любой ответ Bot API (даже 401) — прокси работает; ошибки nginx, не-JSON, таймаут, обрыв соединения — нет.
  Не зависит от панели. Алерты шлются напрямую через api.telegram.org, не через прокси.

При старте в чат уходит сводка: Telegram-прокси, панель, ноды, результат проверки доступа из РФ.
...
node_checker/
  ...
  checks.py       HealthCheck: PanelCheck, NodeStatusCheck, TelegramProxyCheck, BlockingCheck
  tgproxy.py      клиент проверки прокси к Telegram Bot API
